	...
	: QMainWindow(parent)
{
	...
	connect(insertButton, &QPushButton::clicked, this, [=]() 
		...
		if (ok && !nodeName.isEmpty() && search(root, name) == NULL)
		{
			...
			if (ok && size > 0) 
			{
				Chunk* temp = my_alloc(&head, max_heap, size);
				if (temp != NULL)
				{
	...
	connect(deleteButton, &QPushButton::clicked, this, [=]() {
		...
		if (ok && !nodeName.isEmpty()) 
		{
			...
			if (pivot != NULL)
			{
				my_free(max_heap, pivot->address);
				root = del(root, name); 
				scene->clear();
				drawChunks(scene);
			}
