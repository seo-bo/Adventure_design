	...
	: QMainWindow(parent)
{
	...
	connect(expandButton, &QPushButton::clicked, this, [=]() {
		...
		if (ok && size > 0)
		{
			Best_expand(&head, min_heap, size);
			scene->clear();
			drawChunks(scene);
		}
		});
	...
	connect(shrinkButton, &QPushButton::clicked, this, [=]()
		{
			...
			if (ok && size > 0)
			{
				Best_shrink(&head, min_heap, size);
				scene->clear();
				drawChunks(scene);
			}
		});
	...
	connect(mergeButton, &QPushButton::clicked, this, [=]()
		{
			Best_merge_sort(&root, &head, min_heap);
			scene->clear();
			drawChunks(scene);
		});
	...
	connect(insertButton, &QPushButton::clicked, this, [=]()
		{
			...
			if (ok && !nodeName.isEmpty() && search(root, name) == NULL)
			{
				...
				if (ok && size > 0)
				{
					Chunk* temp = Best_my_alloc(&head, min_heap, size);
					if (temp != NULL)
					{
	...
	connect(deleteButton, &QPushButton::clicked, this, [=]() {
		...
		if (ok && !nodeName.isEmpty())
		{
			...
			if (pivot != NULL)
			{
				Best_my_free(min_heap, pivot->address);
				root = del(root, name);
				scene->clear();
				drawChunks(scene);
			}
